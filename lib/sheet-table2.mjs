...
/**
 * @param {string} sheetTabName  e.g. "Taylor Morrison", "Pulte"
 ...
 */
export function reconcilePlaceholders(sheetCounts, dbPlaceholders) {
  const toIngest  = []
  const removeIds = []
  ...
  // Placeholders must always equal the full Table 2 forSale count.
  // Do NOT subtract realActiveCount — the community card reads placeholders only.
  const dbAvail   = dbPlaceholders.avail.length
  const needAvail = Math.max(0, sheetCounts.forSale)

  if (needAvail > dbAvail) {
    for (let i = dbAvail + 1; i <= needAvail; i++)
      toIngest.push({ lotNumber: `avail-${i}`, status: "for sale" })
