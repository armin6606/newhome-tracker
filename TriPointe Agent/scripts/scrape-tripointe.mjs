...
// Helpers
// ---------------------------------------------------------------------------
// Composite primary ID: communityName (no spaces) + rawLot
...
async function scrapeCommunityPlans(page, comm) {
  ...
  const mirHomes = await page.evaluate(({ commSlug, baseUrl }) => {
    ...
    document.querySelectorAll("a[href]").forEach(el => {
      ...
      // Extract lot/unit number from slug (e.g. "42-main-unit-7" → "7", "42-main" → "42")
      const unitM  = addrSlug.match(/-unit-0*(\d+)$/i)
      const rawLot = unitM ? unitM[1] : (addrSlug.match(/^(\d+)/)?.[1] || null)
