...
/** Show "-" for "Lot N" placeholder addresses (no street address published by builder) */
function displayAddress(address: string | null): string {
  if (!address) return "—"
  if (/^Lot\s+\d+$/i.test(address.trim())) return "—"
  return address
}
            ...
            <Link key={l.id} href={`/listings/${l.id}`} className="bg-white rounded-xl border border-stone-200 p-4 shadow-sm hover:shadow-md transition-shadow block">
              <div className="flex items-start justify-between gap-2 mb-2">
                <div className="min-w-0">
                  <p className="font-semibold text-stone-900 text-sm truncate">{displayAddress(l.address)}</p>
              ...
              <div className="grid border-b border-stone-300 bg-stone-50" style={{ gridTemplateColumns: `8rem repeat(${cmpListings.length}, 1fr)` }}>
                ...
                {cmpListings.map((l, i) => (
                  <div key={l.id} className="px-3 py-3 border-l border-stone-300 text-center">
                    <p className="font-bold text-stone-900 text-sm">{displayAddress(l.address)}</p>
