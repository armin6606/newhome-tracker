...
async function sendMissingPlanAlert(
  ...
  if (!RESEND_API_KEY) {
    console.warn("[ingest] RESEND_API_KEY is not configured; skipping missing-plan alert")
    return
  }
  try {
...
export async function POST(req: NextRequest) {
  ...
  if (!sheetCommunities) {
    return NextResponse.json({
      error: `Cannot fetch Google Sheet tab for "${canonicalBuilder}". Ingest blocked.`,
    }, { status: 400 })
  }
