/**
 * Backfill sheet defaults onto all existing Lennar listings in the DB.
...
async function main() {
  ...
  // Get all active Lennar listings
  const listings = await prisma.listing.findMany({
    where: { community: { builder: { name: "Lennar" } } },
    include: { community: true },
  })

  console.log(`Found ${listings.length} Lennar listings to backfill\n`)
  ...
  for (const listing of listings) {
    ...
    console.log(`  ✓ ${communityName} | ${listing.address} | ${listing.floorPlan} → ${applied.floorPlan ?? "—"} | changes: ${Object.keys(changes).join(", ")}`)
    updated++
  }
