/**
 * kb-home.ts
 * Standalone KB Home scraper.
 * Run: npx tsx scripts/scrapers/kb-home.ts
 */
...
const SHEET_GID = "2063280901"
const WEBSITE_URL = "https://www.kbhome.com"
...
async function scrapeOneCommunity(builderId: number, row: SheetCommunityRow): Promise<{ scraped: number; stats: ChangeDetails; error?: { builder: string; error: string } }> {
  ...
  try {
    ...
    let mapResult = await readKBHomeMap(row.url, row.communityName).catch(async (err: unknown) => {
      console.warn(`  [${BUILDER_NAME}] ${row.communityName}: First attempt failed, retrying in 60s...`)
      await new Promise(r => setTimeout(r, 60_000))
      return readKBHomeMap(row.url, row.communityName)
    })
    ...
    if (firstAttemptTotal === 0) {
      ...
      if (dbCount > 3) {
        ...
        await new Promise(r => setTimeout(r, 60_000))
        mapResult = await readKBHomeMap(row.url, row.communityName).catch(() => mapResult)
      }
    }
