/**
 * melia-homes.ts
 * Standalone Melia Homes scraper.
 * Run: npx tsx scripts/scrapers/melia-homes.ts
 */
...
const SHEET_GID = "1767278823"
const WEBSITE_URL = "https://meliahomes.com"
...
async function scrapeOneCommunity(builderId: number, row: SheetCommunityRow): Promise<{ scraped: number; stats: ChangeDetails; error?: { builder: string; error: string } }> {
  ...
  try {
    ...
    let mapResult = await readMeliaMap(row.url, row.communityName).catch(async (err: unknown) => {
      console.warn(`  [${BUILDER_NAME}] ${row.communityName}: First attempt failed, retrying in 60s...`)
      await new Promise(r => setTimeout(r, 60_000))
      return readMeliaMap(row.url, row.communityName)
    })
    ...
    if (firstAttemptTotal === 0) {
      ...
      if (dbCount > 3) {
        ...
        await new Promise(r => setTimeout(r, 60_000))
        mapResult = await readMeliaMap(row.url, row.communityName).catch(() => mapResult)
      }
    }
