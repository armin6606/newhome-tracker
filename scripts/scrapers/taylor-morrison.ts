/**
 * taylor-morrison.ts
 * Standalone Taylor Morrison scraper.
 * Run: npx tsx scripts/scrapers/taylor-morrison.ts
 */
...
const SHEET_GID = "1100202556"
const WEBSITE_URL = "https://www.taylormorrison.com"
...
async function detectAndApplyChanges(
  ...
  for (const scraped of scrapedListings) {
    ...
    const existing = (scrapedLotKey ? existingByLotNumber.get(scrapedLotKey) : undefined)
...
async function scrapeOneCommunity(builderId: number, row: SheetCommunityRow): Promise<{ scraped: number; stats: ChangeDetails; error?: { builder: string; error: string } }> {
  ...
  try {
    ...
    let mapResult = await readTaylorMorrisonMap(row.url, row.communityName).catch(async (err: unknown) => {
      console.warn(`  [${BUILDER_NAME}] ${row.communityName}: First attempt failed, retrying in 60s...`)
      await new Promise(r => setTimeout(r, 60_000))
      return readTaylorMorrisonMap(row.url, row.communityName)
    })
    ...
    if (firstAttemptTotal === 0) {
      ...
      if (dbCount > 3) {
        ...
        await new Promise(r => setTimeout(r, 60_000))
        mapResult = await readTaylorMorrisonMap(row.url, row.communityName).catch(() => mapResult)
      }
    }
